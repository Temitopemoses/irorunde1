...
const GroupAdminManagement = () => {
  ...
  const fetchGroupAdmins = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/apigroup-admins/', {
        headers: {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch('http://127.0.0.1:8000/apiaccounts/groups/');
      if (response.ok) {
        const data = await response.json();
        setGroups(data);
      }
  ...
  const handleCreateAdmin = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch('http://127.0.0.1:8000/apigroup-admins/create/', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteAdmin = async (adminId, username) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://127.0.0.1:8000/apigroup-admins/${adminId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
