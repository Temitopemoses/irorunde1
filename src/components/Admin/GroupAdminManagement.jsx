...
const GroupAdminManagement = () => {
  ...
  const fetchGroupAdmins = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/group-admins/', {
        headers: {
  ...
  const handleCreateAdmin = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch('http://127.0.0.1:8000/api/group-admins/create/', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteAdmin = async (adminId, username) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await fetch(`http://127.0.0.1:8000/api/group-admins/${adminId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
